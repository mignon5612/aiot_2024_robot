...
class Patrol_manipulator(Node):
    ...
    def update(self):
        if self.prev_time + Duration(seconds=3) > self.get_clock().now():
            ...
            # second move
            self.joint_angles = [-0.002, -0.01, -0.2, 0.2, 0.0]
            if self.stage == 1:
                self.send_request()
                self.stage += 1
        elif self.prev_time + Duration(seconds=9) > self.get_clock().now():
            # second move
            self.joint_angles = [0.3, 0.4, -0.3, -0.4, 0.0]
            if self.stage == 2:
                self.send_request()
                self.stage += 1
        elif self.prev_time + Duration(seconds=12) > self.get_clock().now():
            # second move
            self.joint_angles = [0.1, 0.3, -0.2, 0.5, 0.0]
            if self.stage == 3:
                self.send_request()
                self.stage += 1

        elif self.prev_time + Duration(seconds=15) > self.get_clock().now():
            # second move
            self.joint_angles = [0.2, -0.1, 0.5, -0.6, 0.1]
            if self.stage == 4:
                self.send_request()
                self.stage += 1
        
        elif self.prev_time + Duration(seconds=18) > self.get_clock().now():
        # 여섯 번째 이동
            self.joint_angles = [-0.1, 0.5, -0.6, 1.0, -0.2]  # 다른 각도
            if self.stage == 5:
                self.send_request()
                self.stage += 1

        elif self.prev_time + Duration(seconds=20) > self.get_clock().now():
            self.prev_time = self.get_clock().now()
            self.stage = 0
